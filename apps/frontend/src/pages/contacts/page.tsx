...
export default function ContactsPage() {
   ...
   const { onlineUsers, typing, unreadMessages, chatDetails } = useAppSelector((state) => state.chat);
   ...
   return (
      <div className="block md:grid grid-cols-12 gap-3 h-full">
                  ...
                  <React.Fragment>
                     {contacts?.map((contact, idx) => {
                        ...
                        return (
                           <div
                              key={`contact-${idx}`}
                              className="flex items-center p-2 px-3 gap-3 bg-white hover:bg-[#F0F2F5] border-b-[1px]"
                              role="button"
         ...
         {!chatDetails && (
            <div className="hidden col-span-9 h-full md:flex justify-center items-center">
               <img src="/images/chat.webp" alt="empty-chat" className="block" />
            </div>
         )}
      </div>
   );
