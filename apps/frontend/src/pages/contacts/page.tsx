...
export default function ContactsPage() {
   ...
   const { onlineUsers, typing, unreadMessages } = useAppSelector((state) => state.chat);
            ...
            {loadings.getContacts ? (
               ...
               <React.Fragment>
                  {contacts?.map((contact, idx) => {
                     ...
                     return (
                        <div key={`contact-${idx}`} className="flex items-center p-2 px-3 gap-3 bg-white hover:bg-[#F0F2F5] border-b-2" role="button" onClick={() => navigate("/chat/" + contact._id)}>
                           <div className="basis-1/6 flex justify-center">
